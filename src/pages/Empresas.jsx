        ...
        <main>
          <section className="relative overflow-hidden px-6 pb-20 pt-32 md:pb-28 md:pt-40">
            ...
            <div className="relative mx-auto grid max-w-6xl items-center gap-14 lg:grid-cols-[1.12fr_0.88fr] lg:gap-20">
              <div>
                <div data-hero-item className="mb-7 inline-flex items-center gap-2 rounded-full border border-[var(--accent-20)] bg-[var(--accent-10)] px-3 py-1.5 text-xs font-semibold tracking-wide text-[var(--accent)]">
                  {content.hero.badge}
                </div>
                <h1 data-hero-item className="max-w-4xl text-5xl font-semibold leading-[1.05] tracking-tighter md:text-6xl lg:text-7xl">
                  {content.hero.title}{" "}
                  <span className="text-[var(--accent)]">{content.hero.highlight}</span>
                </h1>
                <p data-hero-item className="mt-7 max-w-2xl text-base leading-7 text-[var(--text-secondary)] md:text-lg md:leading-8">
                  {content.hero.description}
                </p>
                <div data-hero-item className="mt-9 flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <div data-hero-item className="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-xs text-[var(--text-secondary)]">
          ...
          <section data-reveal className="border-y border-[var(--border-subtle)] bg-[var(--bg-card-alt)] px-6 py-8">
            <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 md:flex-row">
              <p data-reveal-item className="max-w-md text-center text-base font-semibold leading-snug sm:text-lg md:text-left">{content.segments.title}</p>
              <div className="grid w-full grid-cols-2 gap-3 md:w-auto md:grid-cols-4">
                {content.segments.items.map((item, index) => {
                  ...
                  return (
                    <div key={item} data-reveal-item className="flex min-h-14 items-center justify-center gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--glass-bg)] px-5 py-4 text-sm font-medium text-[var(--text-secondary)]">
          ...
          <section id="solutions" data-reveal className="border-b border-[var(--border-subtle)] bg-[var(--bg-card-alt)] px-6 py-24 md:py-32">
            <div className="mx-auto max-w-6xl">
              ...
              <div className="mt-12 grid gap-6 lg:grid-cols-3">
                {content.solutions.items.map((item, index) => {
                  ...
                  return (
                    <article key={item.title} data-reveal-item className="flex h-full flex-col rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-card)] p-7 transition-colors hover:border-[var(--accent-30)]">
          ...
          <section data-reveal className="border-b border-[var(--border-subtle)] px-6 py-24 md:py-32">
            <div className="mx-auto grid max-w-6xl gap-14 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
              ...
              <ol className="space-y-4">
                {content.process.steps.map((step, index) => (
                  <li key={step.title} data-reveal-item className="flex gap-4 rounded-xl border border-[var(--border-subtle)] bg-[var(--glass-bg)] p-5 backdrop-blur-[10px] transition-colors hover:border-[var(--accent-30)] sm:gap-5 sm:p-6">
          ...
          <section data-reveal className="border-b border-[var(--border-subtle)] bg-[var(--bg-card-alt)] px-6 py-24 md:py-32">
            <div className="mx-auto max-w-5xl">
              ...
              <div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--border-subtle)] sm:grid-cols-2 lg:grid-cols-4">
                {content.why.items.map((item, index) => {
                  ...
                  return (
                    <div key={item.title} data-reveal-item className="bg-[var(--bg-card)] p-6 text-center">
            ...
            <div className="mx-auto max-w-3xl">
              ...
              <div className="mt-10 space-y-3">
                {content.faq.items.map((item) => (
                  <details key={item.question} data-reveal-item className="group rounded-xl border border-[var(--border-subtle)] bg-[var(--glass-bg)] px-5 py-4 open:border-[var(--accent-30)] open:bg-[var(--surface-muted)]">
          ...
          <section data-reveal className="relative overflow-hidden px-6 py-24 md:py-32">
...
function BusinessPreview({ content }) {
  return (
    <div data-business-preview data-hero-item className="relative mx-auto w-full max-w-lg lg:mx-0">
      ...
      <div className="glass-panel relative overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-card)] p-5 shadow-2xl md:p-7">
        <div data-preview-item className="mb-7 flex items-center justify-between border-b border-[var(--border-subtle)] pb-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          {content.metrics.map((metric, index) => {
            ...
            return <div key={metric.label} data-preview-item className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-muted)] p-4"><Icon className="mb-5 h-5 w-5 text-[var(--accent)]" /><p className="text-sm font-medium">{metric.value}</p><p className="mt-1 text-[11px] leading-4 text-[var(--text-secondary)]">{metric.label}</p></div>;
          })}
        </div>
        <div data-preview-item className="mt-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-muted)] p-4">
